...
function scrollToSection(id: string) {
  if (typeof window !== 'undefined') {
    ...
    if (el) {
      const yOffset = -80; // Ajusta según la altura del header
            ...
            <ul className="absolute right-0 mt-2 w-56 bg-[#232B33] rounded shadow-lg border border-[#6FCF97]/30 py-2 z-50 min-w-max">
              {menuLinks.map((link) => (
                <li key={link.href}>
                  <button
                    onClick={() => scrollToSection(link.href.replace('#', ''))}
                    className="block px-4 py-2 text-white hover:text-[#6FCF97] transition font-medium w-full text-left bg-transparent border-none outline-none cursor-pointer"
                  >
                    {link.label}
                  </button>
                </li>
